...
void updateGain(int cellIdx, vector<Cell>& cells, const vector<Net>& nets,
                map<int, unordered_set<int>, greater<int>>& gainA,
                map<int, unordered_set<int>, greater<int>>& gainB,
    ...
    // Update net counts for all nets connected to this cell.
    for (int netIdx : cell.netIndices) {
        Net &net = const_cast<Net&>(nets[netIdx]);
        if (fromA) {
            net.numA--;
            net.numB++;
        } else {
    ...
    // For each net, update the gains of neighboring cells.
    for (int netIdx : cell.netIndices) {
        const Net &net = nets[netIdx];
        long F_new = fromA ? net.numA : net.numB;
        long T = fromA ? net.numB - 1 : net.numA - 1;
        for (int cIdx : net.cellIndices) {
            if (cIdx == cellIdx || cells[cIdx].isLocked) continue;
            int oldGainC = cells[cIdx].gain;
            int deltaGain = 0;
            if (fromA) {
            ...
            if (deltaGain != 0) {
                cells[cIdx].gain = oldGainC + deltaGain;
                auto &gainMap = cells[cIdx].isA ? gainA : gainB;
                gainMap[oldGainC].erase(cIdx);
                if (gainMap[oldGainC].empty())
                    gainMap.erase(oldGainC);
                gainMap[cells[cIdx].gain].insert(cIdx);
            }
        }
    }
    maxGain = max(gainA.empty() ? INT_MIN : gainA.begin()->first,
...
int main(int argc, char* argv[]) {
    ...
    // For demonstration, we just output the number of cells and nets read.
    cout << "Cells: " << cells.size() << ", Nets: " << nets.size() << "\n";
